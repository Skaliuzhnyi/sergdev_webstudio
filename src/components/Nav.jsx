        ...
        <div style={{ width:'100%', maxWidth:1240, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'1rem' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'.625rem' }}>
            ...
            <div style={{ display:'flex', gap:2, background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:8, padding:3 }}>
              {LANGS.map(l => (
                <button key={l} onClick={() => changeLang(l)} style={{ background: lang===l ? 'var(--accent)' : 'none', color: lang===l ? '#000' : 'var(--text2)', fontSize:'.72rem', fontWeight:700, letterSpacing:'.07em', padding:'4px 9px', borderRadius:5, transition:'all .2s', border:'none', cursor:'pointer', fontFamily:'var(--fb)' }}>
                  {l.toUpperCase()}
                </button>
              ))}
            </div>
            ...
            {/* Theme toggle */}
            <button onClick={toggleTheme} style={{ background:'var(--bg3)', border:'1px solid var(--border)', width:34, height:34, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'.9rem', cursor:'pointer' }}>
              {dark ? '🌙' : '☀️'}
            </button>

            {/* CTA — desktop only */}
            <Link to="/contact" className="nb-cta" style={{ background:'var(--accent)', color:'#000', fontSize:'.8125rem', fontWeight:700, padding:'7px 17px', borderRadius:8, transition:'all .2s', whiteSpace:'nowrap', alignItems:'center' }}>
              {t('nav.cta')}
            </Link>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity:0, y:-12 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-12 }} transition={{ duration:.22 }}
            ...
            {['about','services'].map(id => (
              <button key={id} onClick={() => goto(id)} style={{ fontSize:'1.5rem', fontFamily: lang==='uk'?"'Manrope',sans-serif":"'Syne',sans-serif", fontWeight:700, color:'var(--text2)', background:'none', border:'none', cursor:'pointer', textAlign:'left' }}>
                {t(`nav.${id}`)}
              </button>
            ))}
